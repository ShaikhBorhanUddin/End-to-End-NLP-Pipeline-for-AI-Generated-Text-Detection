...
The third notebook (distillbert_model_train.ipynb) implements a transformer-based inference pipeline built around Hugging Face Transformers and a pretrained DistilBERT model to perform sentiment analysis rather than end-to-end supervised training. Specifically, it uses DistilBertForSequenceClassification initialized from the distilbert-base-uncased checkpoint, a compact and computationally efficient variant of BERT, configured with num_labels=2 for binary classification. The architecture consists of a DistilBERT encoder (DistilBertModel) that generates contextualized token embeddings, followed by a linear pre-classifier layer and a final classification head that outputs logits for the two classes. Text inputs are passed through a sentiment analysis pipeline to infer labels, and when predictions are missing, ambiguous, or unavailable, a rule-based proxy sentiment mapping is applied as a fallback mechanism. The model and inputs are automatically moved to the CUDA device when available to accelerate inference. Final sentiment labels from both sources are merged and then aggregated at a higher level, such as by restaurant or title, to compute summary statistics like positive sentiment ratios, positioning this notebook as a representation-learning and inference-driven analytical pipeline rather than a fully trained, task-specific classifier. 
